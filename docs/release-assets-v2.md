...
| --- | --- | --- | --- |
| `v2.0-transform-demo.gif` | 5–15 seconds, stable basic-canvas viewport | select → resize → Shift aspect-lock resize → align/distribute → rotation-deferred feedback | Run the basic-canvas demo, use clean seeded nodes, record deliberate pointer movement, and verify playback before citing it. |
...
| `v2.0-transform-rotation-boundary.png` | Stable basic-canvas viewport | honest V2.0 limitation | Select a node, press the rotation handle, and capture the deferred persistent-rotation status without changing scene JSON. |
...
| Preview-only rotation | `UnsupportedPersistentRotationError`; example status feedback | Core rotation tests and deferred-rotation E2E |
