...
`exportScene()` creates the current version 4 JSON payload. `importScene(json)`
validates it before returning the scene and automatically migrates V1–V3 data,
including legacy edges to canonical connectors. Invalid or unsupported payloads
throw an error; keep the current scene until import succeeds, then call
`canvas.setScene(restored)`, which clears history so undo and redo cannot cross
the import boundary.
...
Current support includes rectangle, circle, and text nodes; viewport navigation;
selection; keyboard deletion; grid snapping; graph groups; and labelled,
straight or orthogonal port connectors. The basic-canvas example supports
dragging between visible ports or using accessible controls to create and
