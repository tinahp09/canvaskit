...
it('groups and ungroups the current selection through commands that need no arguments', () => {
  ...
  expect(kit.getScene().groups).toEqual([{ id: 'group-1', nodeIds: ['a', 'b'], visible: true, locked: false }])
  expect(kit.undo().groups).toEqual([])
  expect(kit.redo().groups).toEqual([{ id: 'group-1', nodeIds: ['a', 'b'], visible: true, locked: false }])
  expect(kit.executeCommand('ungroup-selection')).toBe(true)
  expect(kit.getScene().groups).toEqual([])
  expect(kit.undo().groups).toEqual([{ id: 'group-1', nodeIds: ['a', 'b'], visible: true, locked: false }])
})
