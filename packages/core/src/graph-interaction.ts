...
import type { CanvasConnector, CanvasNode, CanvasScene } from './model.js'
...
export function hitTestConnector(scene: CanvasScene, point: Point, tolerance = 8): CanvasConnector | undefined {
  const controller = new ConnectorController()
  return [...scene.connectors].reverse().find((connector) => {
    ...
    const route = controller.route(scene, connector)
    return route.some((a, index) => {
...
/** @deprecated Use hitTestConnector. */
export const hitTestEdge = hitTestConnector
